...
#' 
#' @export
assayToBigWigs <- function(x, assay=1, genome=NULL, what="5mC", onlycg=TRUE) { 

  if (is.null(genome)) genome <- unique(genome(x))
  if (length(genome) == 0) stop("No genome detected or provided")
  if (length(genome) > 1) stop("Multiple genomes detected or provided")
  if (onlycg) x <- x[grep("^cg", rownames(x)), ] 

  gr <- granges(x)
  for (i in colnames(x)) { 
    gr$score <- getBeta(x)[, i]
    filename <- paste(colnames(x)[i], what, g, "bw", sep=".")
    export(gr, filename)
